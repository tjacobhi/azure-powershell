...
{
    [Cmdlet("Remove", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "SearchPrivateEndpointConnection", DefaultParameterSetName = ResourceNameParameterSetName, SupportsShouldProcess = true), OutputType(typeof(bool))]
    public class RemoveSearchServicePrivateEndpointConnectionCommand : SearchServiceBaseCmdlet
    {
        [Parameter(
            Position = 0,
            Mandatory = true,
        ...
        public override void ExecuteCmdlet()
        {
            ...
            ConfirmAction(Force.IsPresent,
                string.Format(Resources.RemovePrivateEndpointConnectionWarning, PrivateEndpointConnectionName),
                string.Format(Resources.RemovePrivateEndpointConnection, PrivateEndpointConnectionName),
                PrivateEndpointConnectionName,
                () =>
                {
                    CatchThrowInnerException(() =>
                    {
                        SearchClient.PrivateEndpointConnections.DeleteWithHttpMessagesAsync(ResourceGroupName, ServiceName, PrivateEndpointConnectionName);
                    });
